...
export const BUFFER_PROMPT_TEXT =
  ...
  + 'action="grants" to see what you hold or have issued. File actions address the buffer by mapped name: '
  + 'directories granted with an as name live in a per-pipe buffer namespace (each pipe has its own), '
  + 'action="ls" with no path lists the mapped roots, and read / write / edit / download / upload all take '
  + 'buffer paths like as/sub/file — no grant id needed; paths outside the mapped areas are refused. '
  + 'read pages text (offset is a 1-based line, limit caps lines); edit replaces old_string with '
  + 'new_string in place; write replaces the whole file. download and upload are the explicit byte moves '
