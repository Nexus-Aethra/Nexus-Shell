...
export const zh = {
  ...
  'settings.summary': '配色：{theme} · 输入辅助：{helpers} · 数据目录：{data}',
  ...
  'settings.group.data': '数据目录',
  'settings.data.label': '数据目录',
  'settings.data.detail': 'dshell 自己保存的文件放这里：会话转录与命令历史、设备凭据、管道缓冲、会话标签、设备挂载点（<目录>/dshell 与 <目录>/dshell-pty）。留着不动则跟随 dsh 的位置（~/.dsh，或部署指定的 DSH_HOME）',
  'settings.data.following': '跟随 dsh 的位置',
  'settings.data.choose': '选择…',
  'settings.data.reset': '恢复默认',
  'settings.data.note': '重启后生效：主机下次启动时切到这个目录，并把设备与密钥、缓冲、标签、转录迁过去。会话工作目录（挂载点）留在原处，已存在的同名目录不会被覆盖',
...
export const en = {
  ...
  'settings.summary': 'Palette: {theme} · Input assists: {helpers} · Data directory: {data}',
  ...
  'settings.group.data': 'Data directory',
  'settings.data.label': 'Data directory',
  'settings.data.detail': "Where dshell keeps its own files: session transcripts and command history, device credentials, pipe buffers, session tags, device mount points (<dir>/dshell and <dir>/dshell-pty). Left alone, this follows dsh's location (~/.dsh, or a deployment's DSH_HOME)",
  'settings.data.following': "Following dsh's location",
  'settings.data.choose': 'Choose…',
  'settings.data.reset': 'Use the default',
  'settings.data.note': 'Takes effect on restart: the host switches to this directory at its next start and moves devices and keys, buffers, tags and transcripts across. Session working directories (mount points) stay where they are, and an existing directory of the same name is never overwritten',
